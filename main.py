...
def normalize_description(desc, known_merchants):
    ...
    desc = re.sub(r'^\*|^\* | \*$', '', desc)  # Remove leading/trailing asterisks
...
if df_global is not None:
    try:
        ...
        print(f"Processed CSV file saved as 'bank_statements_processed.csv'")
        print(f"Loaded {len(df_global)} transactions from {df_global['Date'].min()} to {df_global['Date'].max()}")
    except Exception as e:
